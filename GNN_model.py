...
def running_code(proc_id, n_gpus, args, devices, overall_graph):
    ...
    for curr_data_split_key in all_the_data_splits_we_want_to_run_now:
        ...
        dataloader_test = dgl.dataloading.DistNodeDataLoader(curr_g, {'source': test_idx}, test_sampler, batch_size=64,
                                                             shuffle=True, drop_last=False,
                                                             num_workers=args.num_workers)
        dev_sampler = sampler
        dataloader_dev = dgl.dataloading.DistNodeDataLoader(curr_g, {'source': dev_idx}, dev_sampler, batch_size=64,
                                                            shuffle=True, drop_last=False, num_workers=args.num_workers)
        ...
        article_id_labels_dict_all, article_source_mapping_all, article_id_name_mapping_all = compute_articles_dict(
            args, overall_graph, graph_style, training_set_to_use, curr_data_split_key, labels_dict,
            add_everything=True, computing_based_on_predicted_labels=False, labels_dict_predicted=None)
        ...
        for em_epoch in range(args.n_epochs):
            ...
            if proc_id == 0 and not args.use_loaded_model_for_secondary_em:
                torch.save(model.state_dict(),
                           args.path_to_save_model + str(curr_data_split_key) + '_em_option_after_lp_nc' + str(
                               em_options_to_run))
            ...
            for iteration, (input_nodes, output_nodes, blocks) in enumerate(dataloader_nc):
                ...
                acc = (torch.sum(output_predictions.argmax(dim=1) == output_labels.long()).item()) / len(
                    output_predictions)
            ...
            if proc_id == 0:
                ...
                train_acc, train_loss, train_f1 = do_evaluation(model, curr_g, overall_graph, args,
                                                                test_dataloader=dataloader_nc, loss_fcn=loss_fcn)
                print("Epoch " + str(em_epoch) + " Train Set Accuracy classification: " + str(
                    train_acc) + " Loss: " + str(train_loss) + " test f1 " + str(train_f1))
                if train_acc > best_train_acc:
                    best_train_acc = train_acc
                print("Best train accuracy " + str(best_train_acc))

                test_acc, test_loss, test_f1 = do_evaluation(model, curr_g, overall_graph, args,
                                                             test_dataloader=dataloader_test, loss_fcn=loss_fcn)
                print("Epoch " + str(em_epoch) + " Test Accuracy classification: " + str(test_acc) + " Loss: " + str(
                    test_loss) + " test f1 " + str(test_f1))
                dev_acc, dev_loss, dev_f1 = do_evaluation(model, curr_g, overall_graph, args,
                                                          test_dataloader=dataloader_dev, loss_fcn=loss_fcn)
                print("Epoch " + str(em_epoch) + " Dev Set Accuracy classification: " + str(dev_acc) + " Loss: " + str(
                    dev_loss) + " test f1 " + str(dev_f1))
                ...
                if save_the_model:
                    ...
                    best_acc = test_acc
                    torch.save(model.state_dict(),
                               args.path_to_save_model + str(curr_data_split_key) + '_em_iter1_option_nc_' + str(
                                   em_options_to_run))
                ...
                torch.save(model.state_dict(),
                           args.path_to_save_model + str(curr_data_split_key) + 'best_at_dev' + str(em_options_to_run))
                sys.stdout.flush()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_loaded_model_for_secondary_em', action='store_true',
                        help="True if you want to load the model for inference operators second iteration")
    ...
    parser.add_argument('--only_run_one_data_split', action='store_true',
                        help="True if you want to only run one data split.")
    parser.add_argument('--run_multiple_data_splits', action='store_true',
                        help="True if you want to reset multiple data splits at once in the EM stage.")
    parser.add_argument("--curr_port", type=str, default=None, help='The port to run on. Useful for multi-GPU training')

    parser.add_argument("--save_only_at_good_dev", action='store_true',
                        help="True if you only want to save at good performance on the dev set.")
    parser.add_argument('--save_always', action='store_true', help="True you always want to save the model.")

    parser.add_argument("--just_train_nc", action='store_true',
                        help="True you just want to train node classification. Otherwise we may end up doing inference operators (in the future).")
    ...
    parser.add_argument('--users_that_follow_each_other_path', type=str,
                        default='dataset_release/users_that_follow_each_other.npy')
    parser.add_argument('--followers_dict_path', type=str, default='dataset_release/source_followers_dict.npy')
    parser.add_argument('--source_username_mapping_dict_path', type=str,
                        default='dataset_release/domain_twitter_triplets_dict.npy')

    parser.add_argument('--percentage_of_dev_to_use', type=float, default=0.20,
                        help='How much of the training set we should train on')
    ...
    parser.add_argument("--em_options_to_run_list", type=str, default='2',
                        help="after how many iterations should we do the inference operator process")
    ...
    overall_graph = graph_helper_functions.FakeNewsDataset(save_path=args.path_where_graph_is,
                                                           users_that_follow_each_other_path=args.users_that_follow_each_other_path,
                                                           dataset_corpus=args.dataset_corpus,
                                                           followers_dict_path=args.followers_dict_path,
                                                           source_username_mapping_dict_path=args.source_username_mapping_dict_path,
    ...
    if n_gpus == 1:
        ...
        running_code(0, n_gpus, args, devices, overall_graph)
